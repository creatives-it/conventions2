...
use App\Repository\DomaineCompetenceRepository;
 ...
 * @ORM\Entity(repositoryClass=DomaineCompetenceRepository::class)
 */
class DomaineCompetence
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=Convention::class, mappedBy="domaineCompetence")
     */
    private $conventions;
    ...
    public function addConvention(Convention $convention): self
    {
        if (!$this->conventions->contains($convention)) {
            ...
            $convention->setDomaineCompetence($this);
        }
    ...
    public function removeConvention(Convention $convention): self
    {
        if ($this->conventions->removeElement($convention)) {
            ...
            if ($convention->getDomaineCompetence() === $this) {
                $convention->setDomaineCompetence(null);
            }
        }
